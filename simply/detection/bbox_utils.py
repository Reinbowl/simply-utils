...
def _auto_color(class_name: str) -> tuple[int, int, int]:
    """Deterministic RGB color from class name via hash, clamped to [100, 255]."""
    ...
    *,
    ...
    Args:
        ...
        detections: List of detections in `fmt_in` format. Each detection is a list:
                    pixel: [class_name, x1, y1, x2, y2] or
                           [class_name, conf, x1, y1, x2, y2]
                    norm:  [class_id, cx, cy, w, h] or
                           [class_id, conf, cx, cy, w, h]
        ...
        class_filter: If provided, only draw detections whose class name
                      is in this list.
...
def viz_bboxes(
    ...
    *,
    ...
    Example:
        ...
        ... )
        >>> viz_bboxes(
        ...     "image.jpg", detections, "output/image_viz", class_filter=["car"]
        ... )
    """
    annotated = draw_bboxes(
        image,
